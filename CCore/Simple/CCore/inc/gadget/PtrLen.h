...
struct PtrLen
 {
  ...
  template <class S>
  ulen matchTo(const S src[/* len */]) const { return RangeMatch<T,S>(ptr,src,len); }

  template <class S>
  bool equalTo(const S src[/* len */]) const { return matchTo(src)==len; }

  template <class S>
  bool equal(PtrLen<S> src) const { return len==src.len && equalTo(src.ptr) ; }

  template <class S>
  bool hasPrefix(PtrLen<S> src) const { return len>=src.len && src.equalTo(ptr) ; }

  template <class S>
  bool hasSuffix(PtrLen<S> src) const { return len>=src.len && src.equalTo(ptr+(len-src.len)) ; }
...
auto Range(RangeAccessType auto &&obj) { return Range(obj.getPtr(),obj.getLen()); }

auto Range_const(const RangeAccessType auto &obj) { return Range_const(obj.getPtr_const(),obj.getLen()); }
...
concept RangeableType = requires(T &obj) { Range(obj); } ;

/* concept TypeRangeableType<T,S> */

template <class T,class S> // T ref extended
concept TypeRangeableType = RangeableType<T> && requires(T &obj)
 {
  Ground< PtrLen<S> >( Range(obj) );
 } ;

/* concept ConstRangeableType<T> */

template <class T> // ref extended
concept ConstRangeableType = requires(Meta::ToConst<T> &cobj) { Range_const(cobj); } ;

/* concept TypeRangeableType<T,S> */

template <class T,class S> // T ref extended
concept ConstTypeRangeableType = ConstRangeableType<T> && requires(Meta::ToConst<T> &cobj)
 {
  Ground< PtrLen<const S> >( Range_const(cobj) );
 } ;

/* begin()/end() */

auto begin(RangeAccessType auto &&obj) { return Range(obj.getPtr(),obj.getLen()); }

ulen end(RangeAccessType auto &&) { return 0; }
