...
template <class Algo,class ResultType,class DataType>
concept CrcAlgo2 = UIntType<ResultType> && UIntType<DataType> && requires()
 {
  Ground< ResultType (*)(ResultType,DataType) >( &Algo::Add );

  Ground< ResultType (*)(ResultType) >( &Algo::Mask );
 } ;

template <class Algo>
concept CrcAlgo = requires()
 {
...
struct AlgoCrc24
 {
  ...
  static const ResultType BitMask = 0xFFFFFFu ;
...
class CrcAccumulator
 {
  ...
  public:
   ...
   void addRange(CursorCastType<DataType> auto r)
    {
     ResultType temp=crc;
