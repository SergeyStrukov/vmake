...
class StrMap : NoCopy
 {
   struct Rec
    {
     ...
     CmpResult operator <=> (const Rec &obj) const noexcept { return StrCmp(key,obj.key); }

     bool operator == (const Rec &obj) const noexcept { return key.equal(obj.key); }

     // swap objects

     void objSwap(Rec &rec) noexcept
      {
       Swap(key,rec.key);
       Swap(obj,rec.obj);
      }
  ...
  protected:
   ...
   explicit StrMap(ulen reserve) : map(DoReserve,reserve) {}
