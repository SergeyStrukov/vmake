...
concept ProxyScanType = requires(ScanBase &inp,Proxy obj)
 {
  obj.scan(inp);
 } ;
...
concept ProxyOptScanType = requires(ScanBase &inp,Proxy obj,Opt opt)
 {
  obj.scan(inp,opt);
 } ;
...
struct ScanAdapter<T>
 {
  ...
  static void Scan(ScannerType auto &inp,const char *,const char *,T &t)
   {
    ProxyType proxy(t);
  ...
  static void Scan(ScannerType auto &inp,const AnyType auto &,T &t)
   {
    ProxyType proxy(t);
  ...
  static void Scan(ScannerType auto &inp,T &t) requires ( ProxyScanType<ProxyType> )
   {
    ProxyType proxy(t);
...
struct ScanAdapter<T>
 {
  ...
  static void Scan(ScannerType auto &inp,const char *ptr,const char *lim,T &t)
   {
    OptType opt(ptr,lim);
    ProxyType proxy(t);
  ...
  static void Scan(ScannerType auto &inp,const OptType &opt,T &t)
   {
    ProxyType proxy(t);
  ...
  static void Scan(ScannerType auto &inp,T &t) requires ( ProxyOptScanType<ProxyType,OptType> )
   {
    OptType opt{};
    ProxyType proxy(t);
...
template <class T>
concept ScanableType2 = requires(ScanBase &inp,T &t)
 {
...
template <class T>
concept ScanableType = IsScanableType< Meta::UnRef<T> > ;
...
class ScanfDev<S> : ScanfDevBase
 {
  ...
  private:
   ...
   template <class ... TT>
   void get(Tuple<TT...> &tuple)
    {
     tuple.call( [this] (TT & ...tt) { ( get(tt) , ... ); } );
    }
  ...
  struct ProxyType
   {
    ...
    void scan(ScannerType auto &inp,const AnyType auto &opt)
     {
      ProxySet set;
