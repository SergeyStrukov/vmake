...
auto ArraySwapIns_guarded(A &array,ulen ind,SS && ... ss)
 {
  ...
  if( ind>len ) ArrayInsPosGuardOutOfBorder(ind,len);
...
template <class A>
auto ArrayInsRange(A &array,ulen ind,ulen count,CreatorType< Meta::PtrObjType<decltype(array.getPtr())> > auto creator)
 {
  ulen len=array.getLen();
...
template <class A>
auto ArrayInsRange(A &array,ulen ind,BuilderType< Meta::PtrObjType<decltype(array.getPtr())> > auto builder)
 {
  ulen len=array.getLen();
