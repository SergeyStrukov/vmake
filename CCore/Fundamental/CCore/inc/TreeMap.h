...
concept RBTreeMapKeyTypes = RBTreeKeyTypes<K,KRef> && requires(const KRef &ref) { K(ref); } ;
...
class RBTreeMap : NoCopy
 {
  ...
  public:
   ...
   void objSwap(RBTreeMap<K,T,KRef,Allocator> &obj) noexcept
    {
     Swap(allocator,obj.allocator);
     Swap(root,obj.root);
    }

   explicit RBTreeMap(ToMoveCtor<RBTreeMap<K,T,KRef,Allocator> > obj) noexcept
...
class RadixTreeMap : NoCopy
 {
  ...
  public:
   ...
   void objSwap(RadixTreeMap<K,T,Allocator> &obj) noexcept
    {
   ...
   explicit RadixTreeMap(ToMoveCtor<RadixTreeMap<K,T,Allocator> > obj) noexcept
    : key_range(obj->key_range),
      allocator(ToMoveCtor(obj->allocator)),
      root(Replace_null(obj->root))
    {
