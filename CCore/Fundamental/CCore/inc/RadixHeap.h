...
template <class Mem>
concept HeapMemType = requires(Mem &mem,const Mem &cmem,bool clean,ulen blen,RadixHeapBlock *block)
 {
  { cmem.getMaxStoreLen() } noexcept;

  Ground<ulen>( cmem.getMaxStoreLen() );

  { mem.cleanup(clean) } noexcept;

  { mem.alloc(blen) } noexcept;

  { mem.free(block) } noexcept;

  { mem.extend(block,blen) } noexcept;

  { mem.shrink(block,blen) } noexcept;

  Ground<RadixHeapBlock *>( mem.alloc(blen) );

  Ground<bool>( mem.extend(block,blen) );

  Ground<ulen>( mem.shrink(block,blen) );
 } ;
...
struct RadixHeapBlock
 {
  ...
  bool isLarge() const { return BitTestAll(flags,FirstBit|LastBit); }
  ...
  RadixHeapBlock * unlock()
   {
    ...
    BitMask(flags,FirstBit|LastBit);
...
class RadixHeapStore : NoCopy
 {
  ...
  public:
   ...
   static constexpr ulen MinLen = RadixHeapBlock::Delta()+Align(sizeof (RadixHeapStore_small::Node)) ;

   RadixHeapStore(ulen max_len) : large(Index(Max(max_len,MinLen))) {}

   void ins(RadixHeapBlock *block) noexcept;

   void del(RadixHeapBlock *block) noexcept;

   RadixHeapBlock * find_and_del(ulen len) noexcept;
 };
...
/* class RadixHeap<Mem> */
 ...
 //
 //    RadixHeapBlock * alloc(ulen blen) noexcept;            // blen aligned
 //                                                           // ( blen<=max_store_len ) => ( ret->len<=max_store_len )
 //
 //    void free(RadixHeapBlock *block) noexcept;             // block!=0
 //
 //    bool extend(RadixHeapBlock *block,ulen blen) noexcept; // block!=0, blen aligned, blen>block->len
 //
 //    ulen shrink(RadixHeapBlock *block,ulen blen) noexcept; // block!=0, blen aligned, blen<=block->len , return delta
 //  };
 //
...
class RadixHeap : NoCopy
 {
  ...
  public:
   ...
   Space alloc(ulen len) noexcept;               // ret.len!=0 on success (==memlen)

   ulen getLen(const void *mem) noexcept;        // mem!=0, return memlen

   ulen free(void *mem) noexcept;                // mem!=0, return memlen

   DeltaLen extend(void *mem,ulen len) noexcept; // mem!=0

   DeltaLen shrink(void *mem,ulen len) noexcept; // mem!=0
 };

template <HeapMemType Mem>
Space RadixHeap<Mem>::alloc(ulen len) noexcept
 {
...
template <HeapMemType Mem>
ulen RadixHeap<Mem>::getLen(const void *mem) noexcept
 {
...
template <HeapMemType Mem>
ulen RadixHeap<Mem>::free(void *mem) noexcept
 {
...
template <HeapMemType Mem>
DeltaLen RadixHeap<Mem>::extend(void *mem,ulen len) noexcept
 {
  RadixHeapBlock *block=RadixHeapBlock::Unlock(mem);
...
template <HeapMemType Mem>
DeltaLen RadixHeap<Mem>::shrink(void *mem,ulen len) noexcept
 {
  RadixHeapBlock *block=RadixHeapBlock::Unlock(mem);
