...
template <class TypeSet>
void EraseExtField(const TypeSet &,Place<void>,StructNode *,int)
 {
 }

template <class TypeSet>
auto EraseExtField(const TypeSet &type_set,Place<void> place,StructNode *struct_node,NothingType) -> decltype( type_set.erase(place,struct_node) )
 {
...
struct EmptyTypeSet : NoCopy
 {
  ...
  MapSizeInfo structSizeInfo(StructNode *);

  template <class T>
  bool isStruct(StructNode *) const { return false; }

  void guardFieldTypes(StructNode *) const;
 };
...
/* class TypedMap<TypeSet> */
 ...
 //
 //    [opt] void erase(Place<void> place,StructNode *struct_node) const;
 //  };
 //
